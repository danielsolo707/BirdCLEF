...
class MyCFG(CFG):
    model_settings = [
        {
            'path': f"{settings['model_dir']}/efficientnet_b2_v1_first10.pth",
            'name': 'efficientnet_b2', 'weight':1.5,
        },
        {
            'path': f"{settings['model_dir']}/efficientnet_b3_v1_first10_aug.pth",
            'name': 'efficientnet_b3', 'weight':2.0,
        },

        {
            'path': f"{settings['model_dir']}/tf_efficientnetv2_b2_v1_first10_aug.pth",
            'name': 'tf_efficientnetv2_b2', 'weight':2.0,
        },
        {
            'path': f"{settings['model_dir']}/tf_efficientnetv2_b3_v1_full.pth",
            'name': 'tf_efficientnetv2_b3', 'weight':2.3,
        },
        {
            'path': f"{settings['model_dir']}/tf_efficientnetv2_s_v1_full.pth",
            'name': 'tf_efficientnetv2_s', 'weight':2.5,
        },

        {
            'path': f"{settings['model_dir']}/tf_efficientnetv2_b3_v1_first10.pth",
            'name': 'tf_efficientnetv2_b3', 'weight':2.3,
        },
        {
            'path': f"{settings['model_dir']}/tf_efficientnetv2_s_v1_first10.pth",
            'name': 'tf_efficientnetv2_s', 'weight':2.5,
        },
        
        {
            'path': f"{settings['model_dir']}/tf_efficientnet_lite0_v1_first10.pth",
            'name': 'tf_efficientnet_lite0', 'weight':0.5,
        },
        {
            'path': f"{settings['model_dir']}/tf_efficientnet_lite1_v1_first10.pth",
            'name': 'tf_efficientnet_lite1', 'weight':0.55,
        },
        {
            'path': f"{settings['model_dir']}/tf_efficientnet_lite2_v1_first10.pth",
            'name': 'tf_efficientnet_lite2', 'weight':0.6,
        },
        {
            'path': f"{settings['model_dir']}/tf_efficientnet_lite3_v1_first10.pth",
            'name': 'tf_efficientnet_lite3', 'weight':0.6,
        },
        {
            'path': f"{settings['model_dir']}/tf_efficientnet_lite4_v1_first10.pth",
            'name': 'tf_efficientnet_lite4', 'weight':0.7,
        },
    ]
...
class MyCFG(CFG):
    model_settings = [
        {
            'path': f"{settings['model_dir']}/tf_efficientnetv2_b3_v2_first10.pth",
            'name': 'tf_efficientnetv2_b3', 'weight':2.3,
        },
        {
            'path': f"{settings['model_dir']}/efficientnet_b3_v2_first10.pth",
            'name': 'efficientnet_b3', 'weight':2.0,
        },
    ]
...
class MyCFG(CFG):
    model_settings = [
        {
            'path': f"{settings['model_dir']}/tf_efficientnetv2_s_v3_first10.pth",
            'name': 'tf_efficientnetv2_s', 'weight':2.5,
        },
        {
            'path': f"{settings['model_dir']}/efficientnet_b2_v3_first10.pth",
            'name': 'efficientnet_b2', 'weight':1.5,
        },
    ]
...
def process_audio_file(audio_data, cfg):
    ...
    piece_len = int(cfg.WINDOW_SIZE*cfg.FS)
    ...
    mel_piece_len = (cfg.TARGET_SHAPE[0]*cfg.TEST_DURATION) // cfg.WINDOW_SIZE
    mel_specs = []
...
def torch_to_ov(model, is_tf=False):
    core = ov.Core()
    
    if is_tf:
        dummy_input = torch.randn(60//cfg.TEST_DURATION, 1, 
                                  cfg.TARGET_SHAPE[1], cfg.TARGET_SHAPE[0]*cfg.TEST_DURATION//5),
        torch.onnx.export(
            model,
            dummy_input,
            f"{settings['data_cache_dir']}/model_cache{args.part}.onnx",
            input_names=["x"],
            output_names=['framewise_output'],
            dynamic_axes=None,
        )
        model = f"{settings['data_cache_dir']}/model_cache{args.part}.onnx"
...
class BirdCLEF2025Pipeline:
    ...
    def predict_on_spectrogram(self, audio_path):
        ...
        for model_settings in self.models:
            with torch.no_grad():
                outputs = model_settings['model'](mel_spec_tensor)[0]
    ...
    def run_inference(self):
        ...
        start_i = (args.part*len(test_files)) // args.n_parts
        end_i = ((args.part+1)*len(test_files)) // args.n_parts
        test_files = test_files[start_i:end_i]
    ...
    def run(self):
        ...
        submission_df.to_csv(f"{settings['data_cache_dir']}/submission_{self.cfg.output_i}_part{args.part}.csv", index=False)
...
def save_final_submission(model_cfgs=model_cfgs):
    ...
    for cfg in model_cfgs:
        sub = pd.read_csv(f"{settings['data_cache_dir']}/submission_{cfg.output_i}_part{args.part}.csv")
    ...
    sub.to_csv(f"{settings['data_cache_dir']}/submission_part{args.part}.csv", index=False)
...
if __name__ == "__main__":
    ...
    with open(f"{settings['data_cache_dir']}/finished{args.part}", 'wb') as f:
        pass